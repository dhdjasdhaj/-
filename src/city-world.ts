...
import {loadLandmarkDetails,loadLandmarkCandidates,isCandidateGroundSlab,type LandmarkDetailManifest,type LandmarkCandidateManifest} from './landmark-details.ts';
...
import {CityRooftops} from './city-rooftops.ts';
import {createRoofSurface} from './city-roof-surface.ts';
import {createCityAmbient} from './city-ambient-occlusion.ts';
...
import type {CityPedestrians} from './pedestrians.ts';
import {CityTraffic} from './traffic.ts';
import {CityTrafficSignals} from './city-traffic-signals.ts';
import type {CityEbikes} from './city-ebikes.ts';
...
/** Shadow-map self-shadowing offsets per frustum. Babylon's normalBias shrinks casters along their normal in
 * world metres, so it has to follow the shadow texel: the 520 m street box is .25 m/px and .10 m is enough,
 * but the 2100 m aerial box is 1.03 m/px and the same .10 m left every sunlit facade speckled with acne
 * (random texels failing the depth test) while true tower shadows on the lawns were mostly lost. .8 m
 * (~.8 texel) clears the acne; casters thinner than that (lamp posts, trunks) drop out of the aerial map. */
export const SHADOW_BIAS={street:{normal:.10,depth:.0002},aerial:{normal:.8,depth:.0002}} as const;
/** terrain.glb / roads.glb materials that read the baked city occlusion (src/city-ambient-occlusion.ts). */
export const GROUND_AMBIENT_MATERIALS=new Set(['land','park','pavement','concrete','asphalt','roadline']);
...
export class DrivingWorld{
 ...
 private applyGraphicsQuality(){
  ...
  if(this.shadows.mapSize!==profile.shadowSize)this.shadows.mapSize=profile.shadowSize;this.applyShadowBias();
 ...
 twinMode=false;observerDragMode:'rotate'|'pan'='rotate';
 ...
 architecture!:ReturnType<typeof createArchitectureMaterials>; facadeDiversity!:ReturnType<typeof createFacadeDiversity>; landscape:CityLandscape|null=null; canopy:CityCanopy|null=null; rooftops:CityRooftops|null=null; roofSurface!:ReturnType<typeof createRoofSurface>; ambient!:ReturnType<typeof createCityAmbient>; signage:Awaited<ReturnType<typeof loadLandmarkSignage>>|null=null; wheelRadius=.4404; lastLandscapeAerial=false;
 ...
 groundHeight:(x:number,z:number)=>number=()=>0;detailManifest:LandmarkDetailManifest|null=null;candidateManifest:LandmarkCandidateManifest|null=null;carFill:PointLight|null=null;traffic:CityTraffic|null=null;signals:CityTrafficSignals|null=null;ebikes:CityEbikes|null=null;pedestrians:CityPedestrians|null=null;trafficSources:AbstractMesh[]=[];wheelSpin=0;
 ...
 constructor(public canvas:HTMLCanvasElement){
  ...
  this.shadows=new ShadowGenerator(2048,this.sun);this.shadows.usePercentageCloserFiltering=true;this.shadows.filteringQuality=ShadowGenerator.QUALITY_LOW;this.shadows.bias=SHADOW_BIAS.street.depth;this.shadows.normalBias=SHADOW_BIAS.street.normal;
  ...
  this.sky();this.environment();this.roofSurface=createRoofSurface(this.scene);this.ambient=createCityAmbient(this.scene);
  // Every architecture material (ordinary blocks, near facades, landmark glass) reads the baked city occlusion; roofs sample at their own height.
  this.architecture=createArchitectureMaterials(this.scene,(id,material)=>{if(id==='city-roof')this.roofSurface.attach(material);this.ambient.attach(material,id==='city-roof'?'roof':'wall');});this.facadeDiversity=createFacadeDiversity(this.scene);
  ...
  window.addEventListener('keydown',e=>{if(e.target instanceof HTMLElement&&e.target.closest('input,textarea,select,[contenteditable=true]'))return;if(['KeyW','KeyS','KeyA','KeyD','KeyQ','KeyE','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','Tab'].includes(e.code))e.preventDefault();this.keys.add(e.code);if(this.twinMode)return;if(!this.paused&&['KeyW','KeyS','KeyA','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(e.code))this.cancelAutoDrive('manual-takeover');if(e.repeat)return;if(e.code==='KeyB')void this.toggleFlight();if(e.code==='KeyT')void this.toggleTank();if(e.code==='Space'&&this.flight?.active)this.flight.fire();else if((e.code==='Enter'||e.code==='Space')&&this.controlMode==='tank'&&!this.paused)this.tank?.fire();if(e.code==='KeyH')this.audio.cue('horn');if(e.code==='KeyG')this.toggleAerial();if(e.code==='KeyR'&&!this.observer.active)this.resetRoad();if(e.code==='KeyC'&&!this.observer.active&&this.walk?.active){this.walkFirstPerson=!this.walkFirstPerson;this.onMessage?.(this.walkFirstPerson?'步行 · 第一人称':'步行 · 第三人称');}else if(e.code==='KeyC'&&!this.observer.active){this.view=(this.view+1)%3;this.onMessage?.(['追踪镜头','驾驶舱镜头','远景镜头'][this.view]);}if(e.code==='KeyL')this.toggleLight();if(e.code==='KeyY')this.toggleRain();if(e.code==='KeyV'){if(this.observer.active)this.exitPhoto();else this.enterPhoto({id:'car',name:'绯红海湾 GT',x:this.state.x,z:this.state.z,height:1.45,area:'车辆',excludeRadius:0,arrival:[this.state.x,this.state.z],yaw:this.state.yaw,photoDistance:7.7,photoElevation:.18,photoAngle:this.state.yaw+Math.PI+.65,photoTargetHeight:this.groundHeight(this.state.x,this.state.z)+.65});}if(e.code==='KeyF'){if(this.observer.active)this.exitPhoto();else if(!this.paused)void this.toggleWalking();}});
  ...
  canvas.addEventListener('pointermove',e=>{if(!this.drag||e.pointerId!==dragPointer||!e.buttons)return;const dx=e.clientX-this.pointer[0],dy=e.clientY-this.pointer[1];if(this.observer.active){if(!this.flight?.active){if(e.shiftKey||this.observerDragMode==='pan')this.observer.pan(dx,dy,canvas.clientHeight);else this.observer.rotate(dx,dy);}}else if(this.walk?.active){this.walk.look(dx,dy);}else{this.cameraYaw+=dx*.005;this.cameraPitch=clamp(this.cameraPitch+dy*.002,.02,.8);}this.pointer=[e.clientX,e.clientY];});
  ...
  canvas.addEventListener('touchmove',e=>{if(!touchCentroid||!e.touches.length)return;e.preventDefault();const next=touchPosition(e.touches),dx=next.x-touchCentroid.x,dy=next.y-touchCentroid.y;if(this.observer.active){if(!this.flight?.active){if(this.observerDragMode==='pan')this.observer.pan(dx,dy,canvas.clientHeight);else this.observer.rotate(dx,dy);}}else if(this.walk?.active){this.walk.look(dx,dy);}else{this.cameraYaw+=dx*.005;this.cameraPitch=clamp(this.cameraPitch+dy*.002,.02,.8);}touchCentroid=next;},{passive:false});
  ...
  this.engine.runRenderLoop(()=>{if(document.hidden||(!this.ready&&!this.twinMode))return;const raw=this.engine.getDeltaTime(),dt=Math.min(raw/1000,.05);const begin=performance.now();if(this.ready){if(this.debugSimulation)this.update(dt);if(this.samples.length<36000)this.samples.push(raw);}this.updateMs=this.updateMs*.9+(performance.now()-begin)*.1;const render=performance.now();this.scene.render();this.renderFrames++;this.renderMs=this.renderMs*.9+(performance.now()-render)*.1;});
 ...
 private async load(name:string){const r=await ImportMeshAsync('/city/'+name+'.glb',this.scene);r.meshes[0].rotationQuaternion=Quaternion.Identity();for(const m of r.meshes){m.isPickable=false;m.receiveShadows=true;if(m.material instanceof PBRMaterial){m.material.environmentIntensity=1.0;m.material.forceIrradianceInFragment=true;m.material.maxSimultaneousLights=8;
   // Ground planes of the base city (lawn, pavement, plaza concrete, asphalt, markings) receive the baked city occlusion; water and lamps do not.
   if((name==='terrain'||name==='roads')&&GROUND_AMBIENT_MATERIALS.has(m.material.name))this.ambient.attach(m.material,'ground');}if(m.getTotalVertices())m.freezeWorldMatrix();}this.architecture.applyMeshes(r.meshes,name);this.facadeDiversity.applyMeshes(r.meshes,name);return r;}
 async init(progress:(s:string)=>void){
  progress('正在展开深圳地图');const ambientLoading=this.ambient.load();const response=await fetch('/city/city.json');if(!response.ok)throw Error('地图加载失败');this.data=await response.json();await loadRoadDisplayNames(this.data);const detail=await loadLandmarkDetails(this.data);if(detail){this.groundHeight=detail.heightAt;this.detailManifest=detail.manifest;}this.candidateManifest=await loadLandmarkCandidates(this.data);progress('正在展开深圳山脊');this.mountains=await loadCityMountains(this.groundHeight);this.groundHeight=this.mountains.heightAt;progress('正在铺设公园缓坡');this.groundRelief=await loadCityGroundRelief(this.groundHeight,undefined,false);this.groundHeight=this.groundRelief.heightAt;progress('正在架设跨水桥梁');this.coastal=await loadCoastalInfrastructure(this.data,this.groundHeight);this.groundHeight=this.coastal.heightAt;this.collision=new CityCollision(this.data);this.walk=new CityWalk((x,z)=>this.collision.blocked(x,z)||this.propBlocked(x,z)||this.parkedVehicleBlocks(x,z),(x,z)=>this.walkingSurfaceHeight(x,z));this.windowSources=[];this.state={...this.data.spawn,speed:0,steer:0,distance:0};if(this.twinMode){this.camera.position.set(this.state.x-80,120,this.state.z-120);this.camera.setTarget(new Vector3(this.state.x,0,this.state.z));}
  ...
  // Merged single-object candidates replace the remaining legacy base landmark silhouettes by name prefix; the source-grounded landmark-detail meshes above are never touched here.
  if(this.candidateManifest){progress('正在替换候选地标模型');const prefixes=this.candidateManifest.replacedMeshPrefixes;for(const m of [...this.scene.meshes])if(prefixes.some(p=>m.name.startsWith(p)))m.dispose(false,false);this.landmarks=this.landmarks.filter(m=>!m.isDisposed());const candidates=await this.load('landmark-candidates');for(const m of candidates.meshes){if(!m.getTotalVertices())continue;const box=m.getBoundingInfo().boundingBox;if(isCandidateGroundSlab(m.name,box.maximumWorld.y-box.minimumWorld.y,this.candidateManifest))m.dispose(false,false);else this.landmarks.push(m);}}
  ...
  if(!this.twinMode){progress('正在启动你的车');const ambient=await this.load('traffic-car');this.trafficSources=ambient.meshes.filter(m=>m.getTotalVertices()>0);ambient.meshes[0].setEnabled(false);const metadata=await fetch('/city/vehicle-manifest.json').then(r=>r.json()) as VehicleManifest;this.wheelRadius=metadata.wheelRadius;this.carHeadlightAnchors=headlightAnchors(metadata.recommendedHeadlightAnchorsGame);this.headlights.forEach((l,i)=>l.position.copyFromFloats(...this.carHeadlightAnchors[i]));const car=await this.load('car');car.meshes[0].parent=this.car;this.carMeshes=car.meshes.filter(m=>m.getTotalVertices()>0);for(const m of car.meshes){m.unfreezeWorldMatrix();const q=m.name.match(/(?:wheel|brake)_([lr][fr])_/);if(q&&m instanceof Mesh){m.setPivotPoint(Vector3.FromArray(metadata.wheelCentresGltf[q[1]]));m.rotationQuaternion=null;}}this.vehicleFinish=applyCinematicVehicleFinish(this.scene,this.car,this.carMeshes);this.carMeshes.push(...this.vehicleFinish.meshes);this.cockpit=createCityCockpit(this.scene,this.car,this.carMeshes);this.carMeshes.push(...this.cockpit.meshes);}else{progress('正在准备纯净低空地图');this.car.setEnabled(false);this.vehicleLightRig.setEnabled(false);this.headlights.forEach(light=>light.setEnabled(false));}
  progress('正在种植榕树、棕榈与花境');this.landscape=new CityLandscape(this.scene,this.groundHeight,(x,z,r)=>!!this.bambooCafe?.layout.reserved(x,z,r)||(this.twinMode&&warehouseReserved(x,z,r)));await this.landscape.init();progress('正在栽种榕树、木棉与樟树林冠');this.canopy=new CityCanopy(this.scene,this.groundHeight,(x,z,r)=>!!this.bambooCafe?.layout.reserved(x,z,r)||(this.twinMode&&warehouseReserved(x,z,r)));await this.canopy.init();
  progress('正在布置屋顶设备、水箱与停机坪');await ambientLoading;this.ambient.setMode(this.lightMode);try{const rooftops=new CityRooftops(this.scene);await rooftops.init();this.rooftops=rooftops;rooftops.setMode(this.lightMode);if(rooftops.plan)void this.roofSurface.load(rooftops.plan.lookup);}catch(error){console.warn('Rooftop layer unavailable; base roofs kept',error);}this.landscape.configureRoadside(this.data,{collisionFootprints:this.detailManifest?.collisionFootprints,bridgeCrossings:this.coastal?.manifest.crossings,blocked:(x,z)=>this.collision.blocked(x,z)||this.propBlocked(x,z)||!!this.canopy?.trunkNear(x,z)});progress('正在布置城市座椅');this.streetFurniture=new CityStreetFurniture(this.scene,this.groundHeight);await this.streetFurniture.init();progress('正在安装路口红绿灯');this.signals=new CityTrafficSignals(this.scene,this.groundHeight);await this.signals.init();this.signals.setNight(this.night);if(!this.twinMode){progress('正在停放电动车');const {CityEbikes}=await import('./city-ebikes.ts');this.ebikes=new CityEbikes(this.scene,this.groundHeight);await this.ebikes.init();}
  ...
  progress('正在调试海湾的光与倒影');this.cinematic=await createCinematicLook(this);if(!this.twinMode){this.vehicleMaterials=refineHeroVehicleMaterials(this.scene,this.carMeshes);this.sportDetails=createCitySportDetails(this.scene,this.car);this.sportDetails.setMode(this.lightMode);this.carMeshes.push(...this.sportDetails.meshes);this.scene.onDisposeObservable.addOnce(()=>this.sportDetails?.dispose());}
  if(!this.twinMode){progress('雨停了，正在铺设路边积水');this.rainPuddles=createRainPuddles(this.scene,this.data,this.groundHeight,this.mirror);await this.rainPuddles.readyPromise;this.rainWeather=createCityRainWeather(this.scene,(x,z,radius,limit)=>this.rainPuddles!.nearby(x,z,radius,limit),this.sun,this.hemi);this.rainWeather.setEnabled(this.raining);this.tailLights=createCityTailLights(this.scene,this.car,this.carMeshes,this.vehicleLightRig);}
  else progress('正在准备低空数字孪生场景');
  this.scene.onDisposeObservable.addOnce(()=>{this.rainWeather?.dispose();this.rainPuddles?.dispose();this.cockpit?.dispose();this.tailLights?.dispose();this.streetFurniture?.dispose();this.bambooCorridor?.dispose();this.audio.dispose();this.vehicleMaterials?.dispose();});
  ...
  this.ready=true;if(!this.twinMode)void this.ensureRider();this.car.position.set(this.state.x,.12,this.state.z);this.car.rotation.y=this.state.yaw;this.cameraYaw=this.state.yaw;
 ...
 startTraffic(graph:RoadGraph){const signalHold=(x:number,z:number,yaw:number)=>this.signals?.holdDistance(x,z,yaw)??null;const directionAllowed=(a:number,b:number)=>{const p=graph.nodes[a],q=graph.nodes[b],n=this.collision.nearest((p[0]+q[0])/2,(p[1]+q[1])/2);return !n?.road.oneway||(q[0]-p[0])*Math.sin(n.yaw)+(q[1]-p[1])*Math.cos(n.yaw)>0;};this.autopilot=new CityAutopilot(graph,this.collision,{signalHold});this.traffic=new CityTraffic(graph,this.trafficSources,this.scene,this.groundHeight,directionAllowed);this.traffic.signalHold=signalHold;this.traffic.place(this.state.x,this.state.z);if(this.ebikes){this.ebikes.attachGraph(graph,directionAllowed);this.ebikes.signalHold=signalHold;this.ebikes.place(this.state.x,this.state.z);}this.cull();}
 setTwinMode(enabled=true){
  this.twinMode=enabled;this.keys.clear();this.drag=false;this.cancelAutoDrive('digital-twin-mode');
  if(enabled){
   this.state.speed=0;this.state.steer=0;this.drivingInput={throttle:0,steer:0,handbrake:true};
   this.flightRequest++;this.flight?.stop();this.tank?.setActive(false);if(this.walk)this.walk.active=false;
  }
  this.car.setEnabled(!enabled);this.vehicleLightRig.setEnabled(!enabled);
  this.headlights.forEach(light=>light.setEnabled(!enabled));
  this.tailLights?.update({braking:false,night:this.night,speed:0,enabled:!enabled});
 }
 setObserverDragMode(mode:'rotate'|'pan'){this.observerDragMode=mode;this.keys.clear();this.drag=false;}
 ...
 /** SHADOW_BIAS is authored for a 2048 map; the normal offset follows the texel when the quality preset shrinks it. */
 applyShadowBias(){const bias=this.overviewEffects?SHADOW_BIAS.aerial:SHADOW_BIAS.street;this.shadows.normalBias=bias.normal*2048/this.shadows.mapSize;this.shadows.bias=bias.depth;}
 aerialEffects(active:boolean){if(this.overviewEffects===active)return;this.overviewEffects=active;this.sun.orthoLeft=this.sun.orthoBottom=active?-1050:-260;this.sun.orthoRight=this.sun.orthoTop=active?1050:260;this.sun.shadowMaxZ=active?3200:1200;this.applyShadowBias();this.shadows.getShadowMap()!.refreshRate=active?12:1;this.shadows.getShadowMap()!.resetRefreshCounter();this.syncGraphicsPostEffects();this.cull();}
 ...
 cull(){const p=this.sceneFocus();this.lastCull.set(p.x,0,p.z);const shadowDistance=this.overviewEffects?1800:800;this.sun.position.set(p.x-this.sun.direction.x*shadowDistance,this.groundHeight(p.x,p.z)-this.sun.direction.y*shadowDistance,p.z-this.sun.direction.z*shadowDistance);this.facadeStream?.update(p.x,p.z,this.debugFacades,this.overviewEffects?250:0);const traffic=this.traffic?.meshes.flat()??[];const casters:AbstractMesh[]=[...(this.flight?.active?this.flight.meshes:[]),...this.vehicleMeshes.filter(m=>m.metadata?.castsShadows!==false),...(this.walk?.active?this.rider?.meshes??[]:[]),...traffic,...(this.pedestrians?.casters??[])];const reflect:AbstractMesh[]=[...(this.flight?.active?this.flight.meshes:[]),...(this.mountains?.meshes??[]),...this.vehicleMeshes,...(this.walk?.active?this.rider?.meshes??[]:[]),...this.landmarks,...this.coastalHorizon.flatMap(h=>h.mesh?[h.mesh]:[]),...traffic,...(this.buildingSigns?.meshes??[])];const sky=this.scene.getMeshByName('atmosphere');if(sky)reflect.push(sky);
  ...
  casters.push(...(this.groundRelief?.shadowMeshes(p.x,p.z,this.overviewEffects?1450:650)??[]));casters.push(...(this.landscape?.casters??[]));casters.push(...(this.canopy?.casters??[]));casters.push(...(this.rooftops?.casters??[]));for(const m of this.facadeStream?.shadowMeshes??[])casters.push(m);
 ...
 vegetation(){
  ...
  this.landscape?.update(focus.x,focus.z,this.overviewEffects,true);this.canopy?.update(focus.x,focus.z,this.overviewEffects,true);this.rooftops?.update(focus.x,focus.z,this.overviewEffects,true);
  const plants=new Set([...(this.landscape?.meshes??[]),...(this.canopy?.meshes??[]),...(this.rooftops?.meshes??[])]),shadow=this.shadows.getShadowMap()!;
  shadow.renderList=[...(shadow.renderList??[]).filter(m=>!plants.has(m)),...(this.landscape?.casters??[]),...(this.canopy?.casters??[]),...(this.rooftops?.casters??[])];shadow.resetRefreshCounter();
  this.localLighting?.update(0,this.camera.position,true);
 }
 setLightMode(mode:'sunset'|'night'|'day'){
  this.lightMode=mode;this.night=mode==='night';this.skyMat.setFloat('night',this.night?1:0);
  this.architecture.setMode(mode);this.facadeDiversity.setMode(mode);this.rooftops?.setMode(mode);this.ambient?.setMode(mode);this.signage?.setMode(mode);this.buildingSigns?.setNight(this.night);this.signals?.setNight(this.night);
